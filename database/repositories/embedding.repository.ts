...
// Encapsulates PGVector operations and HNSW similarity searches.
...
export class EmbeddingRepository {
  ...
  /**
   * Stores a single face detection box and its associated 512-dim embedding (legacy interface).
   */
  async storeEmbedding(
    ...
    return this.insertEmbedding(
      photoId,
      embedding,
      { x: box.x1, y: box.y1, width: box.x2 - box.x1, height: box.y2 - box.y1, confidence: 1.0 },
      { modelName: 'buffalo_l', embeddingDimension: 512, faceIndex: 0 }
    );
  }
